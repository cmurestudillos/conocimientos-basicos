<template>
  ...
  >
    ...
    <Collapse v-model="settingsCollapse" accordion>
      ...
      <Panel name="2">
        Configurar los nombres y el orden del tablero.
        <p slot="content">
          <draggable :list="boardsLocal"
            ...
            <div v-for="board in boardsLocal" class="board" :key="board.id">
              ...
              <Tooltip content="Guardar como JSON"
                       placement="left"
                       :transfer="true"
              >
    ...
    <div slot="footer">
      <Button size="large" @click="closeModal">Cerrar</Button>
      <transition name="fade">
        <div class="restart-required" v-if="restartRequired">
          Se necesita Reinicar
        </div>
      </transition>
    </div>
  </Modal>
      ...
      createBackup () {
        ...
        dialog.showSaveDialog({
          ...
          boardsRepository
            ...
            .then(() => {
              vm.$Message.success('Archivo guardado correctamente.')
            })
            .catch((err) => {
      ...
      importBackup () {
        ...
        dialog.showOpenDialog({
          ...
          boardsRepository.importDbFromJSON(filePath[0])
            .then(() => {
              vm.$Message.success('Archivo importado correctamente.')
