<template>
  <div class="container mt-4">
    ...
    <table class="table table-bordered mt-4">
      ...
      <tbody>
        <tr v-for="(item, index) in items" :key="index">
          ...
          <td>
            <span v-if="editIndex !== index">
              <button @click="edit(item, index)" class="btn btn-sm btn-outline-secondary mr-2">Editar</button>
              <button @click="remove(item, index)" class="btn btn-sm btn-outline-secondary mr-2">Eliminar</button>
            </span>
            <span v-else>
              <button class="btn btn-sm btn-outline-secondary mr-2" @click="cancel(item)">Cancelar</button>
              <button class="btn btn-sm btn-outline-secondary mr-2" @click="save(item)">Guardar</button>
            </span>
          </td>
        </tr>
      </tbody>
    ...
    <div class="col-3 offset-9 text-right my-3">
      <button v-show="!editIndex" @click="add" class="btn btn-sm btn-secondary">Nuevo</button>
    </div>
...
export default {
  ...
  filters: {
    money: (value) => new Intl.NumberFormat('id-ID', { style: 'currency', currency: 'EUR' }).format(value)
  },
  ...
  data() {
    return {
      ...
      originalData: null,
      items: [
        { code: '111', name: 'Fulano', description: 'Fulano Fulanez', qty: 1, unit: null, price: 3500, discount: 10, },
        { code: '222', name: 'Mengano', description: 'Mengano Butano', qty: 2, unit: null, price: 5000, discount: 2, },
      ],
      tax: 10,
    }
  },
